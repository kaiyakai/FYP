...
def calculate_accurate(n, num_list, scene_dict, accurate_dict):
    set_shape = (2*n, 2*n, 3)
...
for n in tqdm([1,2,3,4,5,6,7,8], ncols=70):
    accuracy_dict[n] = {}
    for sn in tqdm([53], ncols=70):
        ...
        for num_list in split_list(list(range(256)), 16):
            p = Process(target=calculate_accurate, args=(n, num_list, scene_dict, accurate_dict))
            jobs.append(p)
            p.start()
...
#         print(result_compare_list)
        ...
        accuracy_dict[n][real_back_prob_mean] = accuracy
        df = pd.DataFrame().from_dict(accuracy_dict)
        df.to_csv(f'accuracy_n_change_{real_back_prob_mean}.csv', index=True)